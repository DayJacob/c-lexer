...
static void chunkmesh_finalize_data(struct ChunkMesh *self) {
  ...
  vbo_buffer(self->vbo, self->buffers[DATA].data, 0, self->buffers[DATA].count);
...
static void chunkmesh_finalize_indices(struct ChunkMesh *self) {
  ...
  vbo_buffer(self->ibo, self->buffers[INDICES].data, 0,
...
enum SortKind { SORT_FULL, SORT_PARTIAL };
...
static void chunkmesh_sort(struct ChunkMesh *self, enum SortKind kind) {
  vec3s center = ((struct PositionComponent *)ecs_get(
                      self->chunk->world->entity_view, C_POSITION))
                     ->position;

  struct ChunkMeshBuffer *faces = &self->buffers[FACES],
  ...
  // sort faces
  qsort(faces->data, faces->elements, sizeof(struct Face),
        (int (*)(const void *, const void *))depth_cmp);

  Bitmap moved =
      kind == SORT_FULL ? bitmap_calloc(indices->elements / 6) : NULL;

  // iterate over sorted faces: move them (in order) into the front of the index
  // buffer
  for (size_t i = 0; i < faces->elements; i++) {
    struct Face *face = &((struct Face *)faces->data)[i];
    if (face->indices_base != i * 6) {
      memcpy(&((u32 *)indices->data)[i * 6],
             &((u32 *)t_indices.data)[face->indices_base], 6 * sizeof(u32));
    }
  ...
  if (kind == SORT_FULL) {
    ...
    // iterate over ALL faces, move those that weren't originally moved to the
    // end of the index buffer
    for (size_t i = 0; i < (indices->elements / 6); i++) {
      if (!bitmap_get(moved, i)) {
        memcpy(&((u32 *)indices->data)[self->indices.base.offset +
                                       self->indices.base.count],
               &((u32 *)t_indices.data)[i * 6], 6 * sizeof(u32));
      ...
      case BLOCKMESH_LIQUID:
        ...
        for (enum Direction d = 0; d < 6; d++) {
          ...
          if (params.block->mesh_type == BLOCKMESH_CUSTOM ||
              ...
              (params.block->transparent && params.block_n->transparent &&
            ...
            if (params.block->mesh_type == BLOCKMESH_CUSTOM) {
              ...
              params.block->get_mesh_information(
                  chunk->world, pos, params.direction, &params.offset,
                  &params.size, &uv_offset_px, &uv_size_px);

              // offset UV coordinates
              vec2s uv_offset = glms_vec2_mul(IVEC2S2V(uv_offset_px),
                                              block_atlas->atlas.pixel_unit),
                    uv_size = glms_vec2_mul(IVEC2S2V(uv_size_px),
                                            block_atlas->atlas.pixel_unit);
...
void chunkmesh_prepare_render(struct ChunkMesh *self) {
  if (self->chunk->world->throttles.mesh.count <
    ...
    if (self->flags.dirty) {
      ...
      if (self->flags.persist && self->buffers[INDICES].data != NULL &&
          self->buffers[FACES].data != NULL) {
...
void chunkmesh_render(struct ChunkMesh *self, enum ChunkMeshPart part) {
  ...
  vao_attr(self->vao, self->vbo, 2, 1, GL_UNSIGNED_INT, vertex_size,
           5 * sizeof(f32));
